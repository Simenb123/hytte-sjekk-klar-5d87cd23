...
import { processBase64Chunks } from './processBase64Chunks.ts'
...
serve(async (req) => {
  ...
  try {
    ...
    if (!audio) {
      throw new Error('Ingen lyddata mottatt')
    }

    console.log('Transcribing audio with length:', audio.length);
    ...
    if (!response.ok) {
      ...
      throw new Error(`OpenAI API feil: ${errorText}`)
    }
