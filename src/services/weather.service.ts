  ...
  private static transformWeatherData(data: any, maxDays = 5): WeatherData {
    ...
    for (const dateStr of orderedDates) {
      ...
      forecast.push({
        ...
        condition: this.getConditionFromSymbol(firstItem.data?.next_1_hours?.summary?.symbol_code || 'clearsky_day'),
        icon: firstItem.data?.next_1_hours?.summary?.symbol_code || 'clearsky_day',
